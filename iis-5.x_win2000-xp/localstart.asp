...
' Local visitors only; everyone else is bounced to the Under Construction gateway.
If Not (sServername = "localhost" Or sServerIP = sRemoteIP) Then
	Response.Redirect "iisstart.asp"
Else

' Read this site's default-document list via ADSI, and check whether a real one exists.
...
<table width="500" cellpadding="5" cellspacing="3" border="0" align="center">
  ...
  <p><% If Not bSuccess And Err = 0 Then %>You do not currently have a default Web page established for your
  ...
  Your Web server lists the following files as possible default Web pages: <%=sDefDoc%>. Currently, only iisstart.asp exists. <% End If %><br><br>
